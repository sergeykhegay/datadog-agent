...
func (h *reOrdererHeap) swap(i, j int) int {
	// ensure to keep generation in the time order
	generationI, generationJ := h.heap[i].generation, h.heap[j].generation

	h.heap[i], h.heap[j] = h.heap[j], h.heap[i]

	// once swapped apply the correct generation
	h.heap[j].generation, h.heap[i].generation = generationJ, generationI
